...
class SODENTrainer(object):
    ...
    def calculate_loss_with_group_censoring(self, features, outputs, labels):
        loss_dict = {}
        loss_dict = {}
        for name in self.criterions:
            if name == "":
                ...
                loss_dict[name] = self.criterions[name](features, outputs, labels)
        return loss_dict

    def train_one_step(self, features, labels):
        self.optimizer.zero_grad()
        outputs = self.model(features)
        if 'group_censoring' in self.exp_name:
            loss_dict = self.calculate_loss_with_group_censoring(features, outputs, labels)
        else:
    ...
    def eval_update_one_step(self, features, labels, phase="valid"):
        outputs = self.model(features)
        for metric_name in self.curr_metrics[phase]:
            if metric_name in ["survival_loss", "F_CI_score", "F_CG_score", "CI_score"]:
                self.curr_metrics[phase][metric_name].add(detach(features), detach(outputs),
                                                          detach(labels))
            else:
