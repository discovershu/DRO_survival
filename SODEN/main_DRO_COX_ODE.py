...
from metrics import (BinomialLogLikelihoodMeter, BrierScoreMeter, CIndexMeter,
                     ...
                     QuantileConcordanceMeter, F_CI, F_CG, CI)
...
parser.add_argument("--dataset", default="seer")  #### support, flc, compas, seer
parser.add_argument("--path", default="./data/seer/")  #### support, flc
parser.add_argument("--verbose", type=int, default=2)
parser.add_argument("--debug", action="store_true")
parser.add_argument("--device", default="cuda")
parser.add_argument("--fine_tune", action="store_true")
parser.add_argument("--evaluate", action="store_true", default=False) #True, False
...
parser.add_argument(
    "--model_config_file",
    default="data/hp_configs/seer__rec_mlp__0__model.json",
    help="Suggested format: Dataset_name__model_type__trial_id__model.json") ##FLC 83, 60, 22. Support 0. Seer 0, 17

# Training configuration.
parser.add_argument(
    "--train_config_file",
    default="data/hp_configs/seer__rec_mlp__0__train.json",
...
parser.add_argument("--save_model", action="store_true", default=True) #True, False
parser.add_argument("--model_path", default=None)
parser.add_argument("--save_log", action="store_true", default=True) #True, False
parser.add_argument("--log_path", default=None)
parser.add_argument("--save_raw", action="store_true")
parser.add_argument('--gpuid', type=str, default='0', help='GPU ID')
parser.add_argument('--alpha', type=float, default=1, help='a lower bound on the group proportions e.g. {0.05, 0.1, 0.15, 0.2, 0.3}')
...
if args.dataset == "mimic":
    ...
    for phase in ["train", "valid", "test"]:
        ...
        dataloaders[phase], feature_size = get_mimic_seq_dataloader(
            ...
            random_state,
            is_eval=(phase != "train"))
elif args.dataset in ["support", "flc", "seer", "compas"]:
    assert args.path.strip("/").split("/")[-1] in ["support", "flc", "seer",  "compas"]
    # Prepare dataloaders.
    random_state = np.random.RandomState(seed=0)
    for phase in ["train", "valid", "test"]:
...
max_l = 10.
C=math.sqrt(1 + 2 * ((1 / args.alpha - 1) ** 2))
if func_type in ["rec_mlp"]:
    if args.alpha == 1.0:
        ...
        def survival_loss(outputs, labels):
            batch_loss = -labels * torch.log(
                outputs["lambda"].clamp(min=1e-8)) + outputs["Lambda"]
            foo = lambda eta: C * math.sqrt((F.relu(batch_loss - eta) ** 2).mean().item()) + eta
            opt_eta = sopt.brent(foo, brack=(0, max_l))
            batch_loss = C * torch.sqrt((F.relu(batch_loss - opt_eta) ** 2).mean()) + opt_eta
            return torch.mean(batch_loss)
...
if func_type in ["rec_mlp"]:
    ...
    # if True:
    if args.evaluate:
        metrics["CI_score"] = CI(S_index=args.protect_index)
        metrics["F_CG_score"] = F_CG(S_index=args.protect_index)
        metrics["F_CI_score"]=F_CI()
...
exp_name = SEP.join([exp_name, "alpha_%s" % args.alpha])
...
if not args.evaluate:
    ...
    result_file = "%s.pkl" % SEP.join(
        ["%.6f" % best_earlystop_metric, trainer.exp_name, "sensitive_%d" % args.protect_index])
    ...
    for type in ['valid', 'test']:
        print('-------------------',type,'-------------------')
        for key in list(result['valid'][0].keys()):
            if isinstance(result[type][0][key], list):
                print('{}_{}:'.format(type,key), '{:0.4f}'.format(result[type][0][key][0]))
            else:
                print('{}_{}:'.format(type, key), '{:0.4f}'.format(result[type][0][key]))
